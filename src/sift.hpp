...
std::vector<Keypoint> find_keypoints_and_descriptors(
    const Image &img, double sigma_min = SIGMA_MIN, int num_octaves = N_OCT,
    int scales_per_octave = N_SPO, double contrast_thresh = C_DOG,
    double edge_thresh = C_EDGE, double lambda_ori = LAMBDA_ORI,
    double lambda_desc = LAMBDA_DESC);
